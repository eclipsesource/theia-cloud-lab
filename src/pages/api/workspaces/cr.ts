...
export default function handler(req: NextApiRequest, res: NextApiResponse<WorkspaceCRData[]>) {
  ...
  if (req.method === 'GET') {
    ...
    toBeDeletedWorkspaceNamesArr.forEach((name: string) => {
      k8sApi.deleteNamespacedCustomObject('theia.cloud', 'v1beta', 'theiacloud', 'workspaces', name).then((data: any) => {
        res.status(200).send(data);
      });
    })
  }
  else if (req.method === 'POST') {
    const toBeCreatedWorkspaceObj: WorkspaceCRObj = {
      'kind': 'Workspace',
      'apiVersion': 'theia.cloud/v1beta',
      'metadata': {
        'name': req.body.toBeCreatedWorkspace,
        'namespace': 'theiacloud',
        
      }
    }
    k8sApi.createNamespacedCustomObject('theia.cloud', 'v1beta', 'theiacloud', 'workspaces', toBeCreatedWorkspaceObj).then((data: any) => {
      res.status(200).send(data);
    });
  }
}